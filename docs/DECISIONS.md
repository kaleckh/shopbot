...
- Decision: Shopbot automatically mines trusted multi-brand retailer catalogues for previously unknown vendors, filters products to menswear, scores candidates against taste terms, and presents up to three representative products with provenance. Candidate brands live in a shared-style registry file; follow, occasional, reject, and too-expensive decisions live in the current profile's taste data. The local server refreshes discovery at startup and every six hours without overlapping runs.
- Why: Stockists encode useful brand adjacency, while separating discovery evidence from user decisions gives the future multi-user product a shared crawl layer and private taste layer.
