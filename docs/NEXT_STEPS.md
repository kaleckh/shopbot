...
- Review the 24 Zara and 12 American Eagle PDP-verified cards now in Suggestions. Both source targets are full. Run `npm run scout:web` for bounded refresh work, or add `--fresh` to bypass category cache. If a retailer presents a normal browser challenge, `--headed` can retain ordinary challenge cookies in the gitignored profile; CAPTCHA, login, and access-control bypass remain out of scope.
- Review the 25 automatically discovered brands. Each decision is profile-specific and recoverable; following and occasional brands should become stronger product sources in the next ranking pass.
- Vote the Suggestions `To review` queue. The `For you` model activates at 15 suggestion votes; `+2` adds an item to `Needs verification`, while purchase outcomes on liked items become stronger ranking evidence.
- Next source work: connect sanctioned affiliate feeds using the new JSON/CSV/XML mappings, starting with Urban Outfitters after feed approval. Levi's discovery is solved through 781 public-sitemap leads; its price/stock verification still needs a normal browser session or authorized stockist. Uniqlo remains a headed-browser/authorized-feed target. Also paginate Okayama/Blue Owl and fix Norse DKK-as-USD before republishing.
...
- Work the `Needs verification` filter after strong votes. Leads were priced from collection data, not product pages; promote a `+2` item only after confirming its current PDP price and per-size stock.
...
- The v2 verified batch came from direct agent page-reads, not a watcher run. Verified price and stock expire after seven days; collection-level leads remain visibly unverified at any age and must be rechecked before buying.
